...
char *subjects[] = {
"math",      "portuguese", "geography",
"history",   "biology",    "philosophy",
"sociology", "chemical",   "physics"
};

int qtd_subjects() { return sizeof(subjects) / sizeof(int *); }
...
double *calculate_study_hours_per_subject(int *difficult_in_each_suject) {
  ...
  study_hours_per_difficulty =
      (hours_per_day * days_per_week) / total_difficulty_hours;

  printf("%.2f, %d\n", (hours_per_day * days_per_week), total_difficulty_hours);

  for (i = 0; i < qtd_subjects(); i++) {
    subject_study_hours[i] =
        round_hours(study_hours_per_difficulty * difficult_in_each_suject[i]);
  }
...
void create_study_cicle() {
  ...
  subject_study_hours =
      calculate_study_hours_per_subject(difficult_in_each_suject);
